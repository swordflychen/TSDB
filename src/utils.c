...
{
    char name[512] = { 0 };
    snprintf(name, (sizeof(name) - 1), "%s%s%s.log", g_cfg_pool.log_path, "/",
            g_cfg_pool.log_file);
...
{
    char pidfile[512] = { 0 };
    snprintf(pidfile, (sizeof(pidfile) - 1), "%s%s", g_cfg_pool.work_path,
            "/pidfile");
    ...
    while (patternLen) {
        ...
        case '*':
            ...
            if (patternLen == 1)
                return 1; /* match */
            while (stringLen) {
                if (string_match_len(pattern + 1, patternLen - 1, string,
                        stringLen, nocase))
                    return 1; /* match */
                string++;
                stringLen--;
...
{
    return string_match_len(pattern, strlen(pattern), string, strlen(string),
            nocase);
}
